from flask import Flask, request, render_template,redirect,url_for  # Import jsonify
import numpy as np
import pandas as pd
import pickle
import sqlite3
...
# Load datasets
sym_des = pd.read_csv(r"C:\Users\gopik\OneDrive\Desktop\miniproject2\datasets\symtoms_df.csv")
precautions = pd.read_csv(r"C:\Users\gopik\OneDrive\Desktop\miniproject2\datasets\precautions_df.csv")
workout = pd.read_csv(r"C:\Users\gopik\OneDrive\Desktop\miniproject2\datasets\workout_df.csv")
description = pd.read_csv(r"C:\Users\gopik\OneDrive\Desktop\miniproject2\datasets\description.csv")
medications = pd.read_csv(r"C:\Users\gopik\OneDrive\Desktop\miniproject2\datasets\medications.csv")
diets = pd.read_csv(r"C:\Users\gopik\OneDrive\Desktop\miniproject2\datasets\diets.csv")


# Load the ensemble model
with open(r'C:\Users\gopik\OneDrive\Desktop\miniproject2\models\ensemble_model.pkl', 'rb') as file:
    ensemble_model = pickle.load(file)

# Helper function to retrieve additional information about the disease
def helper(dis):
    desc = description[description['Disease'] == dis]['Description'].values[0] if not description[description['Disease'] == dis].empty else ''
...
# Define symptoms and diseases dictionaries as provided
symptoms_dict = {
...
def get_predicted_value(patient_symptoms):
    ...
    return diseases_list[ensemble_model.predict([input_vector])[0]]
...
def result():
    if request.method == 'POST':
        ...
        print(f"Input symptoms: {symptoms}")  # Debugging output
        dis = get_predicted_value(symptoms)
        ...
        return render_template('result.html', disease=dis, description=desc, precautions=pre, medications=med, diets=die, workouts=wrkout)

# Function to add a contact (you can modify this to save to a database)
def add_contact(name, address, email, phone, message):
    print(f"Contact added: {name}, {address}, {email}, {phone}, {message}")
...
def contact():
    if request.method == 'POST':
        ...
        add_contact(name, address, email, phone, message)
